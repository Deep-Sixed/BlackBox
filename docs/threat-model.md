...
| An observed repository hiding a submodule commit change through `diff.ignoreSubmodules` or a `.gitmodules` `ignore` setting | Diffs run with `--ignore-submodules=none`, which outranks both | changes reported |
