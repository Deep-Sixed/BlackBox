...
swap the commits it diffs, overrides its submodule `ignore`, `core.fileMode`,
`core.ignoreCase` and out-of-tree exclude settings, which could hide gitlink,
mode or new-file changes,
strips inherited `GIT_*` variables and takes no
